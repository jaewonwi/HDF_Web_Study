...
​
	...
	*session객체 (활용도 ^^)
	...
	scope : a.jsp(session.setAtt.."member" 변수 생성)
	    ...
	    session 변수는 (**현재 Webapp  폴더 안에 있는 **모든 페이지**에서 사용 가능**)
...
<body>
	<%
		...
		//emp 객체는 현재 scope(현재 페이지안에서만 사용가능 :page)
		//모든 페이지 emp 객체 사용 : session 객체 변수
		session.setAttribute("empobj",emp);
		//webapp 폴더안에 있는 모든 페이제(jsp) 사용가능
		
		//POINT 
		//include , forwrad (request 를 공유한다 *****~~~~~)
		//a.jsp (b.jsp include)        c.jsp(d.jsp forward)
		//request 객체하나               //requset 객체하나 
		
		request.setAttribute("who", "king");
		//원칙 : Ex18_Object_Scope.jsp 놈만 request.setAttribute("who", "king"); 사용가능
		//forward ... Ex19_board.jsp >> request.getAttribute("who") >> "king" 사용가능
	%>
	<%=emp.getEname() %><hr>
